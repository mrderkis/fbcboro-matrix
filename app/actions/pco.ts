...
interface PCORawTeamMember { 
  ...
  relationships: { 
    ...
    person?: { data?: { id: string } | null; } | null; 
  }; 
}

interface PCOBlockout {
  id: string;
  attributes: { 
    starts_at: string; 
    ends_at: string; 
...
export async function fetchMatrixPlans(count: number): Promise<DashboardPlan[]> {
  ...
  // 1. Fetch Extra Plans (Buffer for filtering)
  ...
  // 3. Fetch Items and Team Members ONLY for the valid plans
  for (const plan of validPlans) {
    const planId = plan.id;
    
    const [teamRes, itemsRes] = await Promise.all([
      fetch(`https://api.planningcenteronline.com/services/v2/service_types/${SERVICE_TYPE_ID}/plans/${planId}/team_members?include=team,person&per_page=100`, { headers, cache: 'no-store' }),
      fetch(`https://api.planningcenteronline.com/services/v2/service_types/${SERVICE_TYPE_ID}/plans/${planId}/items?per_page=100`, { headers, cache: 'no-store' })
    ]);
  ...
  for (const cached of planDataCache) {
    ...
    dashboardPlans.push({
      ...
      items: planItems,
      declined,
      blockouts: Array.from(uniqueBlockoutNames).sort(),
      teams
    });
  }
