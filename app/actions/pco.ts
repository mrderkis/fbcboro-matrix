...
interface PCORawTeamMember { id: string; attributes: { status: 'C' | 'U' | 'D'; team_position_name: string | null; name: string | null; notifications_sent: boolean; }; relationships: { team?: { data?: { id: string } | null; } | null; person?: { data?: { id: string } | null; } | null; }; }
...
// --- ERROR HELPER TO PREVENT APP CRASH ---
function createErrorPlan(title: string, subtitle: string): DashboardPlan[] {
  return [{
    id: 'error-state', date: 'API ERROR', startTime: null, title: title, series: subtitle, isComplete: false, items: [], declined: [], blockouts: [],
    teams: {
      vocalists: { id: 'v', name: 'Vocalists', members: [], isEmpty: true },
      rhythm: { id: 'r', name: 'Rhythm', members: [], isEmpty: true },
      tech: { id: 't', name: 'Tech', members: [], isEmpty: true },
      orchestra: { id: 'o', name: 'Orchestra', members: [], isEmpty: true },
      safety: { id: 's', name: 'Safety', members: [], isEmpty: true }
    }
  }];
}

/**
 * SERVER ACTIONS - IGNORED SETTINGS
 */
...
/**
 * SERVER ACTIONS - MAIN PLANS DASHBOARD
 */
export async function fetchMatrixPlans(count: number): Promise<DashboardPlan[]> {
  // Guard against NaN if count is somehow undefined
  const safeCount = Number(count) || 4;
  const fetchCount = safeCount + 2; 

  const appId = process.env.PCO_APP_ID;
  const secret = process.env.PCO_SECRET;
  if (!appId || !secret) {
    return createErrorPlan("MISSING API KEYS", "Check .env.local on this Mac and restart server.");
  }
  ...
  try {
    const plansRes = await fetch(
      `https://api.planningcenteronline.com/services/v2/service_types/${SERVICE_TYPE_ID}/plans?filter=future&per_page=${fetchCount}&include=plan_times`, 
      { headers, next: { revalidate: 0 } }
    );
    
    // IF PLANNING CENTER BLOCKS US, RENDER THE ERROR ON SCREEN
    if (!plansRes.ok) {
      const errDetails = await plansRes.text();
      return createErrorPlan(`PCO STATUS: ${plansRes.status}`, `Error for ID ${SERVICE_TYPE_ID}: ${errDetails.substring(0, 80)}`);
    }
    ...
    for (const cached of planDataCache) {
      ...
      teamData.data.forEach(member => {
        ...
        const memberObj: TeamMember = { 
          id: member.id, name: member.attributes.name || 'Unknown', 
          position: member.attributes.team_position_name || 'Unassigned', 
          status, notificationsSent: member.attributes.notifications_sent
        };
        ...
        if (status === 'D') declined.push(memberObj);
          ...
          if (teamId === TARGET_TEAMS.vocalists) teams.vocalists.members.push(memberObj);
            ...
            // Forces Safety Team members into S1, S2, S3 slots automatically
    ...
    return createErrorPlan("NETWORK EXCEPTION", error.message);
  }
}
